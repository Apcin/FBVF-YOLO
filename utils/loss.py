...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.MSEmask, self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance = MSEmask, BCEcls, BCEobj, 1.0, h, autobalance
    ...
    def __call__(self, p, pred_mask, targets):  # predictions, targets
        ...
        lmask = torch.zeros(1, device=self.device) # mask loss
        ...
        ##### mask loss
        tar_obj_mask, tar_bg_mask = self.build_tarmask(pred_mask, targets)
        tar_obj_mask = self.max_pool(tar_obj_mask)
        ...
        l_obj_mask = self.MSEmask(tar_obj_mask, pred_obj_mask)*n_pix / (float(sum(sum(sum(sum(tar_obj_mask)))))+1.0)
        l_bg_mask = self.MSEmask(tar_bg_mask, pred_bg_mask)*n_pix / (n_pix-float(sum(sum(sum(sum(tar_obj_mask)))))+1.0)
        ...
        return (lbox + lobj + lcls + lmask) * bs, torch.cat((lbox, lobj, lcls)).detach(), lmask
