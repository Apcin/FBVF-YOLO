    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.amp.autocast(device_type=device.type, enabled=amp):
                ...
                [pred, mask] = pred
                loss, loss_items, lmask = compute_loss(pred, mask, targets.to(device))  # loss scaled by batch_size
                if RANK != -1:
            ...
            if RANK in {-1, 0}:
                ...
                pbar.set_description(
                    ("%11s" * 2 + "%11.4g" * 6)
                    % (f"{epoch}/{epochs - 1}", mem, *mloss, lmask, targets.shape[0], imgs.shape[-1])
                )
...
def parse_opt(known=False):
    ...
    parser.add_argument("--cfg", type=str, default="models/yolov5-mask.yaml", help="model.yaml path")
