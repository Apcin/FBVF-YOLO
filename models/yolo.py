...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        return [x, mask] if self.training else (torch.cat(z, 1), x, mask) if self.export else (torch.cat(z, 1), x, mask)
    ...
    def __init__(self, cfg="yolov5s.yaml", ch=3, nc=None, anchors=None):
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, Segment) else self.forward(x)[0]
