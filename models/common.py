...
class FEM(nn.Module):
    """Feature Enhancement Module from FFCA-YOLO."""
...
class BasicConv(nn.Module):
    """Convolution block used by the FFCA-YOLO FEM."""
...
class FEMConcat(nn.Module):
    """Apply FEM only to the backbone lateral input, then concatenate."""

    def __init__(self, dimension=1, lateral_channels=1):
        super().__init__()
        self.d = dimension
        self.fem = FEM(lateral_channels, lateral_channels)

    def forward(self, x):
        return torch.cat((x[0], self.fem(x[1])), self.d)
...
class SkipConv(nn.Conv2d):
    def __init__(self, in_c, out_c, kernel_size=3, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode="zeros", act=True):
        self.in_c = in_c
        self.out_c = out_c
    ...
    def forward(self, input) -> torch.Tensor:
        ...
        self.act = nn.SiLU().to(device=output.device)# if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
